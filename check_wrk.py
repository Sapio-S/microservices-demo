...
'''
para = np.load("res/param_300.npy", allow_pickle=True).item()
limit_ok = {}
for s in const_dic:
    limit_ok[s] = {}
...
for i in range (170):
    with open("/home/yuqingxie/microservices-demo/wrk_table0805/"+str(i)) as f:
        text = f.read()
        ...
        if response500:
            ...
            for s in para: 
                for p in para[s][i]:
                    ...
                    if (val-min_val)/(max_val-min_val) <= 1/200:
                        limit_ok[s][p] = True
                        # print(s, p, val, min_val)
